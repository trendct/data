...
p <- ggplot(box_towns, aes(type, Total))
p + geom_boxplot() + ggtitle("Total fentanyl deaths by town type")

p <- ggplot(box_towns, aes(factor(type), percapita))
p + geom_boxplot() + ggtitle("Overdose death rate by town type")

dt12 <- fdrugs_total_2012
...
p + geom_boxplot() + ggtitle("Fentanyl death rate in 2012")

p <- ggplot(box_towns, aes(factor(type), percapita2013))
p + geom_boxplot() + ggtitle("Fentanyl death rate in 2013")

p <- ggplot(box_towns, aes(factor(type), percapita2014))
p + geom_boxplot() + ggtitle("Fentanyl death rate in 2014")

p <- ggplot(box_towns, aes(factor(type), percapita2015))
p + geom_boxplot() + ggtitle("Fentanyl death rate in 2015")
...
r
# This line will only work if you have plotly set up
# plotly_POST(r, filename = "r-docs/fentanyl-overdoses")
...
p <- ggplot(box_towns_rate, aes(x=type, y=percapita,fill=year))+
  geom_boxplot()+
  facet_grid(.~year)+
  labs(list(title="fentanyl overdoses by year in Connecticut", x="Year", y="Per capita"))+
...
# this line will only work if you have plotly set up
# plotly_POST(p, filename = "r-docs/fentanyl-overdoses-ct")
...
# This establishes the range to be put in the loop below
...
r
# this line will only work if you have set up plotly
# plotly_POST(r, filename = "r-docs/fentanyl-overdoses-income-boxplots")
...
# This line will only work if you have set up plotly
# plotly_POST(y, filename = "r-docs/fentanyl-overdoses-income-year-boxplots")
