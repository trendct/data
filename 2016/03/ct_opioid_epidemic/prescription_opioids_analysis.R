...
p + geom_boxplot() + ggtitle("Total prescription opioid deaths by town type")


p <- ggplot(box_towns, aes(factor(type), percapita))
p + geom_boxplot() + ggtitle("Rrescription opioid death rate3 by town type")


dt12 <- pdrugs_total_2012
...
r
# This next line only works if you have a plotly acount and have set up access
# plotly_POST(r, filename = "r-docs/prescription-opioid-overdoses")
...
p <- ggplot(box_towns_rate, aes(x=type, y=percapita,fill=year))+
  geom_boxplot()+
  facet_grid(.~year)+
  labs(list(title="Overall overdose by year in Connecticut", x="Year", y="Per capita"))+
...
# Only works if plotly already set up with proper authorizations
# plotly_POST(p, filename = "r-docs/opioid-overdoses-ct")
...
# income
income <- read.csv("median-income.csv", stringsAsFactors=FALSE)
income$category <- "nope"

# This determines the quantiles I should establish in the loop below
...
# This only works if you have plotly already set up
# plotly_POST(r, filename = "r-docs/opioid-overdoses-income-boxplots")
...
plot_ly(income_box_years, x = year, y = percapita, color = category, type = "box") %>%
  layout(boxmode = "group", xaxis=x, yaxis=y)
