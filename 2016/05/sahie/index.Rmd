---
title: "Analyzing uninsured rates in Connecticut and U.S."
author: "Andrew Ba Tran"
date: "May 18, 2016"
output: html_document
---

This is the methodology used for the story: [Uninsured rates dropping in Connecticut and across the country]( http://trendct.org/?p=13086)

Visit the [repo](https://github.com/trendct/data/tree/master/2016/05/sahie) for the data used in this analysis.
...
* Several maps visualizing the county-level data on the U.S. Census Small Area Health Insurance Estimates 2014 [data](http://www.census.gov/did/www/sahie/data/20082014/index.html)
* A chart of the ethnic breakdown of unisured rates in Connecticut
* A chart of the 2015 national figures from the CDC's National Health Interview [Survey](http://www.cdc.gov/nchs/data/nhis/earlyrelease/quarterly_estimates_2010_2015_q1234.pdf)
* The choropleth maps are based on the code from [rud.is](https://rud.is/b/2016/03/29/easier-composite-u-s-choropleths-with-albersusa/)
* The slopecharts are based on the slopegraph.r script from [jkeirstead](https://github.com/jkeirstead/r-slopegraph)
...
Just 2013 and 2014 for now to compare the change.

First, we'll visualize 2014 data.

```{r y2014, fig.width=9, fig.height=5, warning=FALSE, message=FALSE}
...
# Preparing the FIPS code so it can be easily joined to the map
...
# Bringing in the map by county
cmap <- fortify(counties_composite(), region="fips")

## Working adults in 2014
...
## Creating a map looking just at 2014 data
...
## Comparing 2013 to 2014 pt I

```{r function, warning=FALSE, message=FALSE}
## The other maps
## Creating a function that will take variables and spit out comparisons of 2013 and 2014
## Based on the map code above
...
counties_sahie <- function(oldyear, newyear, column="PCTELIG",
  ...
  gg <- gg + scale_fill_viridis(name="Percent point change", option="plasma")
  gg <- gg + coord_proj(us_laea_proj)
  gg <- gg + labs(title=t,
                  subtitle=s,
                  caption=c)
...
## Comparing 2013 to 2014 pt II

```{r chunk2, fig.width=9, fig.height=5, warning=FALSE, message=FALSE}
# These options refer to http://www.census.gov/did/www/sahie/data/files/SAHIE-File-Layout-Overview.pdf
...
## Uninsured rate since 2008

```{r chunk3, fig.height=5, fig.width=9, warning=FALSE, message=FALSE}
# Bringing all the data in from the Census.
# There's probably a more efficient way to loop this but I've gone too far and deadline's approaching. Copying and pasting here we go.
...
# I know, this is ugly. I apologize.
...
gg.form <- plot_slopegraph(df) +
  labs(title="Percent of insured and uninsured in the United States",
       ...
       caption="TrendCT.org\nSOURCE: NCHS, National Health Interview Survey, 1997–2015") 
gg.form <- gg.form + theme(plot.title=element_text(face="bold", size=14, margin=margin(b=6,l=-20)))
gg.form <- gg.form + theme(plot.subtitle=element_text(size=10, margin=margin(b=6)))
gg.form <- gg.form + theme(plot.caption=element_text(size=9, margin=margin(t=16)))
