---
title: "States housing and income analysis"
...
This is the methodology used behind the story: [A mismatch between rents and wages in Connecticut](http://trendct.org/2016/05/31/the-increasingly-high-cost-of-housing-in-connecticut/).
...
A chart used in this analysis is based on code from [rud.is](https://rud.is/b/2016/04/17/ggplot2-exercising-with-ggalt-dumbbells/).
...
### Step 1. Download 50 spreadsheets and 3 different sheets from each

```{r getting_data, warning=FALSE, message=FALSE, comment=FALSE, include=FALSE}
...
### Rework the data, create new data frames

```{r restructuring, warning=FALSE, message=FALSE}
...
write.csv(states_all, "housing.csv")
...
write.csv(states_all2, "wage.csv")
...
write.csv(states_all3, "hours.csv")
...
colnames(states_all) <- c("region", "value")
states_all$region <- str_to_lower(states_all$region)
state_choropleth(states_all, title = "Housing wage needed to afford a 2-bedroom home")
```


```{r chart1, fig.height=14, fig.width=9}
...
```{r map2, fig.height=6, fig.width=9}
